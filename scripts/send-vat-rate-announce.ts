...
async function main() {
  ...
  console.log(`Sending VAT rate update to ${users.length} users...\n`);
  ...
  for (const user of users) {
    try {
      await resend.emails.send({
        from: FROM,
        to: user.email,
        subject: 'Dreambox CRM update — VAT is now 15.5% and editable',
        html: `<!DOCTYPE html>
<html>
    ...
    <tr><td align="center">
      <table width="520" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;border:1px solid #e2e8f0;padding:40px;">
        ...
        <tr><td style="color:#64748b;font-size:14px;line-height:1.6;padding-bottom:20px;">
          Quick tax update &mdash; the VAT rate has changed and is now configurable from Settings.
        </td></tr>
            ...
            <tr><td>
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr><td style="padding:8px 0;font-size:13px;color:#1e293b;line-height:1.5;">
                  <strong>&#10003; Default VAT is now 15.5%</strong> &mdash; up from 15%. New contracts, invoices, quotations, and receipts use the new rate automatically. Subtotal/VAT/Total breakdowns recalculate accordingly.
                </td></tr>
                <tr><td style="padding:8px 0;font-size:13px;color:#1e293b;line-height:1.5;">
                  <strong>&#10003; Example</strong> &mdash; log a monthly rate of <strong>$100</strong> with VAT inclusive, and the contract/invoice now reads: Subtotal $86.58, VAT (15.5%) $13.42, <strong>Total $100</strong>.
                </td></tr>
                <tr><td style="padding:8px 0;font-size:13px;color:#1e293b;line-height:1.5;">
                  <strong>&#10003; The rate is editable</strong> &mdash; open <em>Settings &rarr; Company Profile</em> and you'll find a new <strong>VAT Rate (%)</strong> field next to the VAT Number. Change it once and every label, PDF, and email picks up the new value.
                </td></tr>
                <tr><td style="padding:8px 0;font-size:13px;color:#1e293b;line-height:1.5;">
                  <strong>&#10003; Labels are dynamic</strong> &mdash; the &ldquo;Rate includes VAT (15.5%)&rdquo; checkbox, the totals row on invoices/quotations, and the contract template all reflect whatever rate you set.
                </td></tr>
              </table>
            </td></tr>
          </table>
        ...
        <tr><td style="color:#64748b;font-size:14px;line-height:1.6;padding-bottom:20px;">
          <strong style="color:#1e293b;">Heads up:</strong> existing contracts and invoices keep the figures they were saved with. Only newly created documents use the new rate. If you need an older contract recalculated at 15.5%, edit and re-save it.
        </td></tr>
