...
async function main() {
  const email = 'julietmberitiana@gmail.com';
  const firstName = 'Juliet';
  const lastName = 'Mberitiana';
  const username = 'julietm';

  // Check if already exists
  const existing = await prisma.user.findUnique({ where: { email } });
  if (existing) {
    console.log(`User already exists: ${existing.firstName} ${existing.lastName} (${existing.status})`);
    console.log(`Sending password reset email anyway...`);
    // Still send reset email in case they need it
    await sendResetEmail(existing);
    await prisma.$disconnect();
    return;
  }

  // Generate temp password (strong, 12-char with all requirements)
  ...
  console.log(`✓ User created and reset email sent to ${email}`);
  await prisma.$disconnect();
}
    ...
    <tr><td align="center">
      <table width="480" cellpadding="0" cellspacing="0" style="background:#12121a;border-radius:16px;border:1px solid rgba(255,255,255,0.06);padding:40px;">
        ...
        <tr><td style="color:#475569;font-size:12px;line-height:1.6;border-top:1px solid rgba(255,255,255,0.06);padding-top:24px;">
          This link expires in <strong>24 hours</strong>. If it expires, ask an admin to send a new reset.<br>
