...
const MinimalInput = ({ label, value, onChange, type = "text", required = false }: any) => (
  <div className="group relative pt-5">
    ...
    />
    <label className="absolute left-0 top-0 text-xs text-slate-900 font-medium transition-all peer-placeholder-shown:text-sm peer-placeholder-shown:text-slate-900 peer-placeholder-shown:top-5 peer-focus:top-0 peer-focus:text-xs peer-focus:text-slate-800 uppercase tracking-wide">
        {label}
    </label>
  </div>
);
    ...
    <div className="space-y-8 animate-fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-900 font-medium">Track asset health and 3-month service cycles</p>
        </div>
        <div className="flex gap-3 items-center w-full sm:w-auto">
            <div className="flex bg-slate-100 rounded-full p-1 border border-slate-200">
                <button onClick={() => setViewMode('Schedule')} className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-all ${viewMode === 'Schedule' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-900 hover:text-slate-900'}`}>Schedule</button>
                <button onClick={() => setViewMode('History')} className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-all ${viewMode === 'History' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-900 hover:text-slate-900'}`}>History Log</button>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
              <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                  ...
                  <div className="relative group w-64">
                      <Search className="absolute left-3 top-2.5 text-slate-900" size={16} />
                  ...
                  <table className="w-full text-left text-sm text-slate-900 min-w-[800px]">
                      <thead className="bg-slate-50 border-b border-slate-100">
                          <tr>
                              ...
                              <th className="px-6 py-4 font-bold text-xs uppercase text-slate-900 tracking-wider">Location</th>
                              <th className="px-6 py-4 font-bold text-xs uppercase text-slate-900 tracking-wider">Last Service</th>
                              <th className="px-6 py-4 font-bold text-xs uppercase text-slate-900 tracking-wider">Next Due</th>
                          ...
                          {filteredBillboards.map(b => {
                              ...
                              return (
                                  <tr key={b.id} className="hover:bg-slate-50 transition-colors">
                                      ...
                                      <td className="px-6 py-4 flex items-center gap-1"><MapPin size={14} className="text-slate-900"/> {b.location}, {b.town}</td>
                  ...
                  {logs.sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(log => (
                      <div key={log.id} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all">
                          <div className="flex justify-between items-start mb-4">
                              ...
                              <span className="text-xs font-mono text-slate-900">{log.date}</span>
                          </div>
                          <h4 className="font-bold text-slate-900 mb-1">{getBillboardName(log.billboardId)}</h4>
                          <p className="text-sm text-slate-900 mb-2 line-clamp-2">{log.description}</p>
                          {log.notes && (
                              <div className="bg-slate-50 p-2 rounded-lg border border-slate-100 mb-4">
                                  <p className="text-xs text-slate-900 italic flex items-start gap-1">
                                      <FileText size={12} className="mt-0.5 shrink-0"/> {log.notes}
                                  </p>
                              </div>
                          )}
                          <div className="flex justify-between items-center text-xs font-bold uppercase tracking-wider text-slate-900 border-t border-slate-50 pt-4">
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-50 p-4 transition-all" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div className="bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl max-w-lg w-full border border-white/20 max-h-[90vh] overflow-y-auto">
                ...
                <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-white sticky top-0 z-10">
                    <div>
                        ...
                        <p className="text-xs text-slate-900 mt-0.5">
                            {selectedBillboardId
                                ? <span>Asset: <span className="font-semibold text-slate-900">{getBillboardName(selectedBillboardId)}</span></span>
                ...
                <form onSubmit={handleSave} className="p-8 space-y-6">
                    ...
                    {!selectedBillboardId ? (
                        <div>
                            <p className="text-xs font-bold uppercase tracking-wider text-slate-900 mb-3">Select Asset</p>
                            <MinimalSelect
                            ...
                            />
                            <p className="text-[10px] text-slate-900 mt-2">Choose the billboard this maintenance event applies to. This updates its service schedule.</p>
                        </div>
                    ) : (
                    ...
                    {/* Service record */}
                    <div>
                        <p className="text-xs font-bold uppercase tracking-wider text-slate-900 mb-3">Service Record</p>
                        <div className="grid grid-cols-2 gap-6">
                            <div>
                                <MinimalInput
                                    ...
                                    required
                                />
                                <p className="text-[10px] text-slate-900 mt-1.5">Next service due 3 months from this date.</p>
                            </div>
                            <div>
                                <MinimalSelect
                                    label="Service Type"
                    ...
                    <div>
                        <MinimalInput
                            ...
                            required
                        />
                        <p className="text-[10px] text-slate-900 mt-1.5">Briefly describe what was done — e.g. "Replaced torn vinyl, re-tensioned cables."</p>
                    </div>
                    ...
                    {/* Cost & Technician */}
                    <div>
                        <p className="text-xs font-bold uppercase tracking-wider text-slate-900 mb-3">Cost & Technician</p>
                        <div className="grid grid-cols-2 gap-6">
                            <div>
                                <MinimalInput
                                    label="Cost ($)"
                                ...
                                />
                                <p className="text-[10px] text-slate-900 mt-1.5">Enter 0 if no external cost was incurred.</p>
                            </div>
                            <MinimalInput
                                label="Performed By"
                    ...
                    {(newLog.cost || 0) > 0 && (
                        ...
                        >
                            ...
                            <div>
                                ...
                                <p className="text-[10px] text-slate-900 mt-0.5">Adds a <span className="font-bold text-slate-900">${newLog.cost}</span> entry to General Expenses under "Maintenance" — visible in the Expenses tab.</p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="flex gap-3 pt-2">
                        ...
                        <button
                            type="submit"
                            className="flex-1 py-3 text-white bg-slate-900 hover:bg-slate-800 rounded-2xl font-bold uppercase text-xs tracking-wider transition-all hover:-translate-y-0.5 shadow-md hover:shadow-lg flex items-center justify-center gap-2"
                        >
