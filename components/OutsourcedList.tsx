          ...
          {billboards.length === 0 ? (
            <div className="col-span-full flex flex-col items-center justify-center py-20 text-center">
              ...
              <div key={billboard.id} className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 hover:shadow-lg transition-all group hover:-translate-y-1 duration-300">
                  <div className="flex justify-between items-start mb-4"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-full bg-indigo-50 text-indigo-600 flex items-center justify-center shadow-sm"><Globe size={20} /></div><div><h3 className="font-bold text-slate-900 leading-tight">{billboard.billboardName}</h3><p className="text-xs text-slate-900 font-mono">ID: {billboard.billboardId}</p></div></div><div className="flex gap-2"><button onClick={() => { setCurrentBillboard(billboard); setIsModalOpen(true); }} className="text-slate-300 hover:text-slate-900 transition-colors"><Edit2 size={16} /></button><button onClick={() => setItemToDelete(billboard)} className="text-slate-300 hover:text-red-500 transition-colors"><Trash2 size={16} /></button></div></div>
                  <div className="space-y-4 py-4 border-t border-slate-50"><div><p className="text-[10px] uppercase font-bold text-slate-900 mb-1">Media Owner (Partner)</p><p className="text-sm font-medium text-slate-800">{billboard.mediaOwner}</p><p className="text-xs text-slate-900">{billboard.ownerContact}</p></div><div className="grid grid-cols-2 gap-4"><div className="bg-green-50 p-3 rounded-xl border border-green-100"><div className="flex items-center gap-2 text-green-700 text-xs font-bold uppercase mb-1"><DollarSign size={12} /> Payout/Mo</div><p className="text-lg font-bold text-slate-900">${billboard.monthlyPayout.toLocaleString()}</p></div><div className="bg-slate-50 p-3 rounded-xl border border-slate-100"><div className="flex items-center gap-2 text-slate-900 text-xs font-bold uppercase mb-1"><Calendar size={12} /> Ends</div><p className="text-sm font-bold text-slate-800">{billboard.contractEnd}</p></div></div></div>
              </div>
          ))}
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-50 p-4 transition-all">
            <div className="bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl max-w-lg w-full border border-white/20 max-h-[90vh] overflow-y-auto">
                <div className="p-6 border-b border-slate-100 flex justify-between items-center sticky top-0 bg-white z-10">
                    ...
                    <button onClick={() => { setIsModalOpen(false); setCurrentBillboard({}); }} className="p-2 hover:bg-slate-100 rounded-full transition-colors"><X size={20} className="text-slate-900" /></button>
                </div>
                <form onSubmit={handleSave} className="p-8 space-y-6">
                    ...
                    {isEditing && (
                        <div className="bg-slate-900 text-white rounded-2xl p-5">
                            <p className="text-xs font-bold uppercase tracking-wider text-slate-900 mb-2">Current Assignment</p>
                            ...
                            <div className="grid grid-cols-2 gap-4 border-t border-slate-700 pt-3 mt-3">
                                <div>
                                    <p className="text-xs font-bold uppercase tracking-wider text-slate-900 mb-1">Partner</p>
                                    <p className="text-sm text-slate-300">{currentBillboard.mediaOwner || '—'}</p>
                                </div>
                                <div>
                                    <p className="text-xs font-bold uppercase tracking-wider text-slate-900 mb-1">Monthly Payout</p>
                    ...
                    {/* Billboard Selection */}
                    <div>
                        <p className="text-xs font-bold uppercase tracking-wider text-slate-900 mb-4">Billboard Asset</p>
                        <MinimalSelect
                        ...
                        />
                        <p className="text-[10px] text-slate-900 mt-3">Only billboards already registered in Inventory can be outsourced.</p>
                    </div>

                    {/* Partner Details */}
                    <div>
                        <p className="text-xs font-bold uppercase tracking-wider text-slate-900 mb-4">Partner Details</p>
                    ...
                    <div className="bg-slate-50 rounded-2xl border border-slate-100 p-5 space-y-5">
                        <p className="text-xs font-bold uppercase tracking-wider text-slate-900">Contract Terms</p>
                        <div>
                            ...
                            <p className="text-[10px] text-slate-900 mt-2">Amount paid to the media owner each month for use of this asset.</p>
                        </div>
                        <div className="grid grid-cols-2 gap-6">
                            <div>
                        ...
                        <p className="text-[10px] text-slate-900">Period during which this outsourced agreement is active.</p>
                    </div>
            ...
            <div className="bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl max-w-sm w-full border border-white/20">
                ...
                <div className="p-6 space-y-4">
                    ...
                    <div className="bg-slate-50 rounded-xl border border-slate-100 p-4 space-y-1.5">
                        <p className="text-xs font-bold uppercase tracking-wider text-slate-900 mb-2">Assignment Being Removed</p>
                        <p className="font-bold text-slate-900">{itemToDelete.billboardName}</p>
                        <p className="text-sm text-slate-900 flex items-center gap-2"><Globe size={13} className="text-slate-900 shrink-0" /> {itemToDelete.mediaOwner}</p>
                        {itemToDelete.ownerContact && (
                            <p className="text-xs text-slate-900">{itemToDelete.ownerContact}</p>
                        )}
                        <p className="text-xs text-slate-900 flex items-center gap-2"><DollarSign size={12} className="text-slate-900" /> ${itemToDelete.monthlyPayout.toLocaleString()}/month payout</p>
                        <p className="text-xs text-slate-900 font-mono mt-1">ID: {itemToDelete.id}</p>
                    </div>
                    {/* Impact warning */}
