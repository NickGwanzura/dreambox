...
export const Payments: React.FC = () => {
    ...
    const confirmPayment = async () => {
        if (selectedInvoice) {
            ...
            logAction('Payment', `Invoice #${selectedInvoice.id} paid — $${(selectedInvoice.total ?? 0).toLocaleString()} from ${clientName} via ${paymentDetails.method}${paymentDetails.reference ? ` (Ref: ${paymentDetails.reference})` : ''}`);
            refreshInvoices();
                            ...
                            <div className="p-8 space-y-6">
                                ...
                                <div className="flex gap-3 pt-1">
                                    ...
                                    <button
                                        ...
                                        className="flex-1 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-bold uppercase tracking-wider text-xs transition-all hover:-translate-y-0.5 shadow-md hover:shadow-lg shadow-emerald-600/20 flex items-center justify-center gap-2"
                                    >
                    ...
                    <div className="bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl max-w-md w-full border border-white/20 overflow-hidden max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="p-8 space-y-6">
                            <div className="bg-slate-50 rounded-2xl p-6 text-center border border-slate-100">
                                ...
                                <h2 className="text-4xl font-extrabold text-slate-900 tracking-tighter">${(selectedInvoice.total ?? 0).toLocaleString()}</h2>
                            ...
                            <div className="flex gap-3">
                                ...
                                <button onClick={confirmPayment} className="flex-1 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-bold uppercase tracking-wider text-xs transition-all hover:-translate-y-0.5 shadow-md hover:shadow-lg shadow-emerald-600/20 flex items-center justify-center gap-2"><Check size={15} /> Confirm Payment</button>
                            </div>
                        </div>
                    </div>
                </div>
