...
import { getClients, getInvoices, getClientFinancials, getTransactions, getContracts, getBillboards, addInvoice, getUpcomingBillings, deleteInvoice, logAction } from '../services/mockData';
...
export const Payments: React.FC = () => {
    ...
    const [paymentDetails, setPaymentDetails] = useState({ method: 'Bank Transfer', reference: '', date: new Date().toISOString().split('T')[0], amount: 0, receivedBy: defaultReceiver, receivingAccount: '' });
    ...
    const outstandingAmount = (invoice: Invoice) => {
        const paid = allReceipts.filter(r => !r.isVoided && r.linkedInvoiceId === invoice.id).reduce((sum, r) => sum + Number(r.total || 0), 0);
        return Math.max(0, Math.round((Number(invoice.total || 0) - paid) * 100) / 100);
    };
    ...
    const confirmMonthlyPayment = async () => {
        ...
        const { month, year, amount, date, method, reference, receivedBy, receivingAccount } = monthlyForm;
        if (!receivedBy.trim()) { alert('Who received the payment is required.'); return; }
        if (!reference.trim()) { alert('Payment reference is required.'); return; }
        if (isBankPaymentMethod(method) && !receivingAccount.trim()) { alert('Receiving bank account is required for bank payments.'); return; }
        if (isBankPaymentMethod(method) && !monthlyProofFile) { alert('Attach proof of payment before posting a bank payment.'); return; }
        let proof = null;
        if (monthlyProofFile) {
            setProofUploading(true);
            try { proof = await uploadPaymentProof(monthlyProofFile); }
            catch (e: any) { alert(e?.message || 'Proof upload failed.'); return; }
        ...
        if (!invoiceExists) {
            ...
            const invoice: Invoice = {
                ...
                date: date,
                items: [{ description: `Monthly Rental — ${monthLabel} (${monthlyContract.details || getBillboardName(monthlyContract.billboardId)})`, amount }],
                subtotal,
                vatAmount,
                total: amount,
                status: 'Pending',
    ...
    const handleOpenPaymentModal = (invoice: Invoice) => { setSelectedInvoice(invoice); setPaymentProofFile(null); setPaymentDetails({ method: 'Bank Transfer', reference: '', date: new Date().toISOString().split('T')[0], amount: outstandingAmount(invoice), receivedBy: defaultReceiver, receivingAccount: '' }); };
    const confirmPayment = async () => {
        if (selectedInvoice) {
            if (!paymentDetails.receivedBy.trim()) { alert('Who received the payment is required.'); return; }
            ...
            if (amount - outstanding > 0.01) { alert(`Payment exceeds the outstanding balance of $${outstanding.toFixed(2)}.`); return; }
            if (!paymentDetails.reference.trim()) { alert('Payment reference is required.'); return; }
            if (isBankPaymentMethod(paymentDetails.method) && !paymentDetails.receivingAccount.trim()) { alert('Receiving bank account is required for bank payments.'); return; }
            if (isBankPaymentMethod(paymentDetails.method) && !paymentProofFile) { alert('Attach proof of payment before posting a bank payment.'); return; }
            let proof = null;
            if (paymentProofFile) {
                setProofUploading(true);
            ...
            const receipt: Invoice = { id: `RCT-${Date.now()}`, clientId: selectedInvoice.clientId, date: paymentDetails.date, items: [{ description: `Payment for Invoice #${selectedInvoice.id}`, amount }], linkedInvoiceId: selectedInvoice.id, subtotal: amount, vatAmount: 0, total: amount, status: 'Paid', type: 'Receipt', contractId: selectedInvoice.contractId, paymentMethod: paymentDetails.method as any, paymentReference: paymentDetails.reference, receivedBy: paymentDetails.receivedBy, receivingAccount: paymentDetails.receivingAccount || undefined, proofPaymentUrl: proof?.url, proofOriginalName: proof?.originalName, proofMimeType: proof?.mimeType, proofUploadedAt: proof?.uploadedAt };
            try {
                await addInvoice(receipt);
            } catch (err: any) {
                alert(`Failed: ${err?.message || 'Server error. Please try again.'}`);
            ...
            logAction('Payment', `Invoice #${selectedInvoice.id} payment — $${amount.toLocaleString()} from ${clientName} via ${paymentDetails.method} (Ref: ${paymentDetails.reference})`);
            refreshInvoices();
            setSelectedInvoice(null);
            setPaymentProofFile(null);
            setPaymentDetails({ method: 'Bank Transfer', reference: '', date: new Date().toISOString().split('T')[0], amount: 0, receivedBy: defaultReceiver, receivingAccount: '' });
        }
    };

    const handleDeleteInvoice = async (invoice: Invoice) => {
        const reason = window.prompt(`Void Invoice #${invoice.id}? Enter the audit reason:`);
        if (reason) {
            try {
    ...
    const handleDeleteReceipt = async (receiptId: string) => {
        const reason = window.prompt('Reverse this payment? The original receipt and proof will be preserved. Enter the audit reason:');
        if (reason) {
            try {
                                    ...
                                    {upcomingBillings.map((item, index) => (
                                        <tr key={index} className="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td className="px-6 py-4">
                                                <span className="px-3 py-1 bg-indigo-50 text-indigo-700 rounded-xl text-xs font-bold uppercase">{item.day} of month</span>
                                            </td>
                            ...
                            <div className="p-8 space-y-6">
                                ...
                                <div className="space-y-3">
                                    ...
                                    <MinimalInput
                                        ...
                                        required
                                    />
                                    <MinimalInput label="Received By" value={monthlyForm.receivedBy} onChange={(e: any) => setMonthlyForm(f => ({ ...f, receivedBy: e.target.value }))} required />
                                    {isBankPaymentMethod(monthlyForm.method) && <MinimalInput label="Receiving Bank Account" value={monthlyForm.receivingAccount} onChange={(e: any) => setMonthlyForm(f => ({ ...f, receivingAccount: e.target.value }))} required />}
                                    <div><label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-2">Proof of Payment {isBankPaymentMethod(monthlyForm.method) ? '*' : '(Optional)'}</label><input type="file" accept="application/pdf,image/jpeg,image/png,image/webp" required={isBankPaymentMethod(monthlyForm.method)} onChange={e => setMonthlyProofFile(e.target.files?.[0] || null)} className="block w-full text-xs file:mr-3 file:rounded-xl file:border-0 file:bg-slate-900 file:px-3 file:py-2 file:font-bold file:text-white"/><p className="mt-1 text-[10px] text-slate-600">PDF/JPEG/PNG/WebP, up to 7 MB.</p></div>
                                ...
                                <div className="flex gap-3 pt-1">
                                    ...
                                    >
                                        <Check size={15} /> {proofUploading ? 'Uploading…' : 'Confirm Payment'}
                                    </button>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl max-w-md w-full border border-white/20 overflow-hidden max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="p-8 space-y-6">
                            <div className="bg-slate-50 rounded-2xl p-6 text-center border border-slate-100">
                                ...
                                <h2 className="text-4xl font-extrabold text-slate-900 tracking-tighter">${outstandingAmount(selectedInvoice).toLocaleString()}</h2>
                            ...
                            <div className="space-y-3">
                                ...
                                <MinimalInput label="Amount Received" type="number" value={paymentDetails.amount} onChange={(e: any) => setPaymentDetails({...paymentDetails, amount: Number(e.target.value)})} required />
                                ...
                                <MinimalInput label="Reference Number" value={paymentDetails.reference} onChange={(e: any) => setPaymentDetails({...paymentDetails, reference: e.target.value})} icon={Hash} placeholder="e.g. POP-12345" required />
                                <MinimalInput label="Received By" value={paymentDetails.receivedBy} onChange={(e: any) => setPaymentDetails({...paymentDetails, receivedBy: e.target.value})} required />
                                {isBankPaymentMethod(paymentDetails.method) && <MinimalInput label="Receiving Bank Account" value={paymentDetails.receivingAccount} onChange={(e: any) => setPaymentDetails({...paymentDetails, receivingAccount: e.target.value})} required />}
                                <div><label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-2">Proof of Payment {isBankPaymentMethod(paymentDetails.method) ? '*' : '(Optional)'}</label><input type="file" accept="application/pdf,image/jpeg,image/png,image/webp" required={isBankPaymentMethod(paymentDetails.method)} onChange={e => setPaymentProofFile(e.target.files?.[0] || null)} className="block w-full text-xs file:mr-3 file:rounded-xl file:border-0 file:bg-slate-900 file:px-3 file:py-2 file:font-bold file:text-white"/><p className="mt-1 text-[10px] text-slate-600">PDF/JPEG/PNG/WebP, up to 7 MB. Recorder and upload time are logged automatically.</p></div>
                                <p className="text-[10px] text-slate-900">A receipt will be issued and the invoice marked paid.</p>
                            </div>
                            <div className="flex gap-3">
                                <button onClick={() => setSelectedInvoice(null)} className="flex-1 py-3 text-slate-900 bg-slate-100 hover:bg-slate-200 rounded-2xl font-bold uppercase text-xs tracking-wider transition-all hover:-translate-y-0.5">Cancel</button>
                                <button onClick={confirmPayment} disabled={proofUploading} className="flex-1 py-3 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-60 text-white rounded-2xl font-bold uppercase tracking-wider text-xs transition-all hover:-translate-y-0.5 shadow-md hover:shadow-lg shadow-emerald-600/20 flex items-center justify-center gap-2"><Check size={15} /> {proofUploading ? 'Uploading…' : 'Confirm Payment'}</button>
                            </div>
                        </div>
                    </div>
                </div>
