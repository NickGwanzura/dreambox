...
export default async function handler(req: HttpRequest, res: HttpResponse) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      const [rows, total] = await Promise.all([prisma.cRMEmailThread.findMany({
        where,
        orderBy: [{ createdAt: 'asc' }, { id: 'asc' }],
        take, skip,
      }), typeof prisma.cRMEmailThread.count === 'function' ? prisma.cRMEmailThread.count({ where }) : Promise.resolve(0)]);
